    ...
    public void onInitialize() {
        ...
        if ( !FileIO.GlobalTextPath.toFile().exists() || !FileIO.GlobalTextPath.toFile().isDirectory() ) {
            try {
                ...
                throw new CrashException(new CrashReport("Cannot create global text data directory ", e));
            }
        }
        ...
        Identifiers.TextFileNetworkingIdentifier = new Identifier("textfilereader", "networking/textfile");
        Identifiers.DebugFileIdentifier = new Identifier("textfilereader", "debug/test");
        Tasks.TaskPool_Server = new LinkedList<>();
        ServerPlayNetworking.registerGlobalReceiver(Identifiers.TextFileNetworkingIdentifier,
                (server, player, handler, buf, responseSender) -> Tasks.GetNetPackageCallback(server, player, handler, buf, responseSender, Identifiers.TextFileNetworkingIdentifier));
        ServerPlayNetworking.registerGlobalReceiver(Identifiers.DebugFileIdentifier,
                (server, player, handler, buf, responseSender) -> Tasks.GetNetPackageCallback(server, player, handler, buf, responseSender, Identifiers.DebugFileIdentifier));
